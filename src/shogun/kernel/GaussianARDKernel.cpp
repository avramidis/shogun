		...
		const std::string param_name, index_t index)
{
	...
	if (lhs==rhs)
	{
		if (!param_name.compare("log_weights"))
		{
		...
		for (index_t j=0; j<length; j++)
		{
			if (!param_name.compare("log_weights") )
			{
				...
				else
				{
					...
					derivative[j]=get_parameter_gradient_helper(param_name,index,j,j,avec,bvec);
				}
...
float64_t CGaussianARDKernel::get_parameter_gradient_helper(
	const std::string param_name, index_t index, int32_t idx_a,
		...
		for (index_t j=0; j<num_lhs; j++)
		{
			...
			for (index_t k=0; k<num_rhs; k++)
			{
				...
				else
				{
					...
					derivative(j,k)=get_parameter_gradient_helper(param_name,index,j,k,avec,bvec);
				}
			}
		}
		return derivative;
